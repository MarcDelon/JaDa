...
export default function BabyPage() {
  const [babyClothes, setBabyClothes] = useState<any[]>([])
  ...
  const fetchBabyProducts = async () => {
    ...
    if (error) {
      ...
      setBabyClothes(formattedProducts)
    }
    setLoading(false)
  }
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Bébé (0-36 mois)</h1>
          <p className="text-muted-foreground text-lg">Vêtements doux et confortables pour bébés</p>
        </div>

        {loading ? (
          <div className="text-center py-20">
            <p className="text-lg text-muted-foreground">Chargement des produits bébé...</p>
          </div>
        ) : babyClothes.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-2xl font-semibold mb-2">Aucun produit disponible</p>
            <p className="text-muted-foreground">Aucun produit bébé (0-36 mois) disponible pour le moment</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {babyClothes.map((product) => (
