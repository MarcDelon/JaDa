...
export default function ClothingPage() {
  ...
  const fetchClothingProducts = async () => {
    ...
    if (error) {
      ...
      const formattedProducts = data.map(product => ({
        ...
        category: product.subcategory || 'Vêtement',
        color: product.color || 'Non spécifié',
        size: product.size || []
      }))
      setClothingProducts(formattedProducts)
    }
    setLoading(false)
  }
  ...
  const colors = Array.from(new Set(clothingProducts.map((p) => p.color)))
  const categories = Array.from(new Set(clothingProducts.map((p) => p.category)))
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-3">
            <div className="mb-8">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">Vêtements Femme</h1>
              <p className="text-muted-foreground text-lg">{sortedProducts.length} articles disponibles</p>
            </div>

            {loading ? (
              <div className="text-center py-20">
                <p className="text-lg text-muted-foreground">Chargement des vêtements...</p>
              </div>
              ...
              <div className="text-center py-20">
                <p className="text-2xl font-semibold mb-2">Aucun vêtement disponible</p>
                <p className="text-muted-foreground">Aucun vêtement femme disponible pour le moment</p>
              </div>
            ) : (
