...
export default function ShoesPage() {
  ...
  const fetchShoes = async () => {
    ...
    if (error) {
      ...
      const formattedProducts = data.map(product => ({
        ...
        category: product.type || 'Chaussures',
        color: product.color || 'Non spécifié',
        size: product.sizes || []
      }))
      setShoesProducts(formattedProducts)
    }
    setLoading(false)
  }
  ...
  const colors = Array.from(new Set(shoesProducts.map((p) => p.color)))
  const categories = Array.from(new Set(shoesProducts.map((p) => p.category)))
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 border-b border-border">
        <div className="flex items-center space-x-2 text-sm text-muted-foreground">
          ...
          <span>/</span>
          <span className="text-foreground">Chaussures</span>
        </div>
      </div>
